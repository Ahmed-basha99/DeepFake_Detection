...
def train_models(
    ...
    #train the model on both the datasets together
...
def train_raw_net(
    ...
    # train fake models
    # print(f"number of fake_training_distributions is {len(fake_training_distributions)}")
    # for current in fake_training_distributions:
    #     # if current in[
    #     #     Path("/home/g9/ahmed/WaveFake/dataset/generated_audio/ljspeech_multi_band_melgan"),
    #     #     Path("/home/g9/ahmed/WaveFake/dataset/generated_audio/ljspeech_full_band_melgan"), 
    #     #     Path("/home/g9/ahmed/WaveFake/dataset/generated_audio/ljspeech_hifiGAN")

    #     # ]:
    #     #     LOGGER.info(f"Skipping, trained before: {current}")
    #     #     continue
    #     LOGGER.info(f"Training {current}")
    #     fake_dataset_train, _ = load_directory_split_train_test(
    #         current,
    ...
    #     LOGGER.info(
    #         f"Training rawnet model on ({len(data_train)},{len(data_train[0])}) audio files.")
        
    #     pos_weight = torch.tensor(
    #         len(fake_dataset_train) / len(real_dataset_train))
    ...
    print ("train on both datasets")
    fake_dataset = fake_training_distributions 
        # equal amount of real and fake data
    all_fake_datasets = list(map(lambda x: load_directory_split_train_test(
            x,
            None,
            None,
            test_size,
    ...
    data_train = ConcatDataset(
            [real_dataset_train, *all_fake_datasets])
    LOGGER.info(
            f"Training rawnet model on {len(data_train)} audio files.")

    leave_one_out_size = sum(map(len, all_fake_datasets))
    pos_weight = torch.tensor(
