...
def evaluate_raw_net(
    ...
    print(f"Size: {len(real_dataset_test)}")
    print(f"before path {current_model_path}")
    ...
    LOGGER.info(f"calculatnig EER for model trained on new dataset")
    fake_out_results = {}
    for fake_dir_out in map(str, fake_dirs):
        if fake_dir_out == fake_dir:
            continue
...
def main(args):
    ...
    # print(f" fake model: {fake_models}")
    if len(fake_dirs) == 0:
        fake_dirs = [base_dir]
    
    if args.raw_net:
        print(f"Model: {fake_models}")
        evaluate_raw_net(
