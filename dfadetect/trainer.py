...
class GDTrainer(Trainer):
    def train(self,
        ...
        # pos_weight= torch.FloatTensor([0.1, 0.9]).to(self.device)
        criterion = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        optim = self.optimizer_fn(model.parameters(), lr=0.0001, weight_decay= 0.0001)
        ...
        for epoch in range(1, self.epochs):
            ...
            LOGGER.info(
                f"[{epoch:04d}/{self.epochs:04d}]: {running_loss} - train acc: {train_accuracy} - test_acc: {test_acc}")
